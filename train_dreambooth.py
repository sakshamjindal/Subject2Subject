...
def log_validation(
    ...
    if args.text_inv_dir is not None:
        pipeline.load_textual_inversion(args.text_inv_dir)
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            grid = image_grid(images, rows=args.num_validation_images//3, cols=3)
            #np_images = np.stack([np.asarray(img) for img in images])
            tracker.writer.add_images("validation", grid, epoch, dataformats="HWC")
